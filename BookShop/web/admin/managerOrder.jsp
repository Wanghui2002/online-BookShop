...
<body>
	...
	<script type="text/javascript">
		function fahuo(ordernum) {
			window.location.href = "../order/OrderServlet?op=faHuo&ordernum="
					+ ordernum;
			alert("发货成功！！");
		}
	</script>
		...
		<c:forEach items="${orders}" var="os" varStatus="vs">
			<tr class="active">
				...
				<c:if test="${os.status==1}">
					<td>已发货</td>
					<td><a href="${pageContext.request.contextPath}/order/OrderServlet?op=deletes&orderNum=${os.ordernum}">删除</a></td>
				</c:if>
			</tr>
		</c:forEach>
	</table>
