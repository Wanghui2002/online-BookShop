	...
	<c:if test="${empty sessionScope.cart.itmes}">
		<div style="margin-left: 600px; margin-top: 100px">
			...
			<h4 style="margin-left: 170px">
				<strong>空空如也！快去把喜欢的宝贝添加进购物车吧！！！</strong> <a href="${pageContext.request.contextPath}/client/ClientServlet?op=category">返回主页</a>
			</h4>
		</div>
	</c:if>
	<c:if test="${!empty sessionScope.cart.itmes}">
			...
			<c:forEach items="${sessionScope.cart.itmes}" var="c" varStatus="vs">
				<tr class="active">
					...
					<td><img style="width: 80px; height: 100px"
						src="${pageContext.request.contextPath}/images/${c.value.book.path}/${c.value.book.filename}" />
					</td>
			...
			<tr>
				<td colspan="10" style="padding-left: 1450px"><strong>总数量：<span
					...
					<button type="button" class="btn btn-danger"
						...
						onclick="window.location='${pageContext.request.contextPath}/order/OrderServlet?op=genOrder'">
